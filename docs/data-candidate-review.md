...
审核更新：2026-09-07（应用 6.0.0 / 分析 2.0.0；原始数据未改写）
...
- 训练：8 个单元、352 个观测；
- 已查看开发比较：4 个单元、176 个观测；源 `validation` 角色保留供追溯，本版不称未触碰测试；
- 禁止按行随机拆分同一轨迹；曲线标签不是独立生物实验认证。
...
- 在参数和选模冻结后进行已查看开发集比较，明确其不具备未触碰测试或 L4 证据资格。
...
训练与原 `validation` 角色按完整曲线标签分开，后者数值不参与当前训练拟合、观测层剖面化、CV 选模或 bootstrap。但后者在此前版本已查看，只能用于当前开发比较；同管分孔不等于独立实验，准入表没有充分记录：
...
2026-09-07 的补充审查没有准入新的三药时间序列。能阅读论文与能取得许可明确、可解析且条件映射完整的数据是不同状态：

- **Li et al. 2021**：保留为 BW25113 无药批量增长线索，方法、机器可读控制曲线和许可仍待核实；未内置。
- **Otoupal et al. 2021**：[公开全文](https://pmc.ncbi.nlm.nih.gov/articles/PMC8571399/)，[DOI](https://doi.org/10.1038/s42003-021-02783-x)。潜在 BW25113 单药对照；附件 `42003_2021_2783_MOESM4_ESM.xlsx` 获取返回 challenge，未得到可读取并核对的菌株/药物/浓度/时间映射。没有绕过挑战，没有声称该数据已验证或准入。
- **Vaisbourd et al. 2025**：[公开全文](https://pmc.ncbi.nlm.nih.gov/articles/PMC12607625/)。主要菌株为 MG1655，BW25113 子集、三药覆盖、独立性及可重用文件仍待逐项核实；未准入。
- **Broughton et al. 2025**：[公开全文](https://pmc.ncbi.nlm.nih.gov/articles/PMC12759072/)。涉及改造荧光菌株，不能当作本项目未改造目标菌株的匹配三药数据；独立单药对照与可重用数据仍待核实，未准入。

论文方法复用与实现差异见 [数学模型与论文证据](./research-method-evidence.md)。未准入候选不进入运行参数估计或“真实验证结果”。
