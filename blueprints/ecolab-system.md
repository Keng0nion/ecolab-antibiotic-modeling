...
能力等级按**每次分析运行**评估，而不是给整个应用永久贴标签。Learn / Sandbox 属于 **L1 — 可复现探索**。内置研究工作流可报告 **L3 — 数据校准**，但该标签仅表示完成拟合/分析/诊断步骤，不证明优化收敛、参数识别或预测有效。原留出已在此前版本查看，6.0.0 只报告开发集比较；孔/板独立性也未充分记录，不具备 L4。完成、收敛、可识别性和统计精度须分别报告，以上等级都不构成通用实验预测或临床能力。
...
### 第 5 步：完善、审计与作品集发布（历史 5.0.0）

以下记录属于当时的发布线与 238 项测试，不是 6.0.0 本轮验证。历史工件保持原样；当前状态见下一节和发布检查表。
...
**批准：** 用户于 2026-09-07 选择 B：保留教学模型及历史数据，分版本修复研究与工程语义，不因新增方法而制造三药验证证据。

**实施：** 应用 `6.0.0`，分析 `2.0.0` / `ecolab-research-analysis-v2`；稳定分析 ID 保留。修复 Nelder–Mead 预算边界、Morris 归一化、Sobol 配对 bootstrap/预算上限、目标切片与秩亏 covariance 语义；修复存储降级/冲突和导出/恢复关联。增加直接 OD Logistic/Gompertz、整训练轨迹 CV、联合 bootstrap、完整输入研究包及仅内置同版显式回放。回放同时比较科学投影与完整 manifest；预览不覆盖当前数据。

**证据规则：** 旧 `validation` 数据角色保留追溯，本版评价是已查看开发比较。参数模型未收敛、基线获选与统计精度不足必须如实保留；Morris/Sobol 输入范围和两类 bootstrap 不可混淆。三药仍为跨条件教学药参，没有新增条件匹配抗生素数据。

**当前验收：** 实际测试、构建、哈希与浏览器状态以 [发布检查表](../docs/release-checklist.md) 为唯一当前记录；[升级计划](../docs/research-upgrade-plan.md) 记录实施检查项，[方法证据](../docs/research-method-evidence.md) 记录论文与代码映射。本轮不提交 Git、不推送、不部署；人工矩阵与 P95 不冒充已完成。
...
- 研究能力包括导入、拟合、敏感性、不确定性、训练 CV 和冻结开发比较；能力及收敛/识别/精度按运行评估；
- 严格保持现有生物机制范围；
- 初始五步按审批门推进；后续 B 方案已获批准，仍不扩展非目标生物机制。
